...
def torn_polygon(cx, cy, w, h, seed, jitter=0.06, points=24):
    ...
    for i in range(points):
        angle = (2 * 3.14159265 * i) / points
        rx = (w / 2) * (1 + rnd.uniform(-jitter, jitter))
        ry = (h / 2) * (1 + rnd.uniform(-jitter, jitter))
        x = cx + rx * (1 if True else 1) * __import__("math").cos(angle)
        y = cy + ry * __import__("math").sin(angle)
        pts.append((x, y))
    return pts
...
def make_torn_piece(image_path, out_name, piece_w, piece_h, seed, border=26):
    ...
    gray = ImageOps.grayscale(canvas.convert("RGB")).convert("RGBA")
    gray.putalpha(canvas.split()[3])
    gray.save(f"{out_name}_bw.png")


make_torn_piece("goya_flagellants.jpg", "piece_flagellants", 780, 500, seed=1)
make_torn_piece("poncher_death.jpg", "piece_death", 480, 640, seed=2)
make_torn_piece("church_iconoclasm.jpg", "piece_church", 680, 480, seed=3)
make_torn_piece("weeping_madonna.jpg", "piece_madonna", 420, 520, seed=4)
